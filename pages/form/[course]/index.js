...
const CFA_TOPICS = [
  'Corporate Finance','Economics','Equity','Ethics','Fixed Income',
  'FRA','Portfolio','Quants','Others',
]
const FRM1_TOPICS = [
...
const FRM2_TOPICS = [
  ...
  'Risk Management & investment Management','Current Issues','Liquidity Risk',
]
...
function Step2CFA({ title, sessionOptions, nextPath }) {
  ...
  const [percentiles, setPercent]   = useState({})
  ...
  const onSubmit = e => {
    ...
    router.push({
      pathname: nextPath,
      query: {
        course, email, name, userId, mobile,
        ...
        <fieldset>
          ...
          <table>
            <thead>
              <tr>
                <th>Topic</th>
                <th>Below 50</th><th>50–70</th><th>Above 70</th>
              </tr>
            </thead>
            <tbody>
              {CFA_TOPICS.map(t=>(
                  ...
                  {['below-50','50-70','above-70'].map(b=>(
                    <td key={b}>
                      <input
                        ...
                        name={t}
                        value={b}
                        checked={percentiles[t]===b}
                        required
                        onChange={()=>setBand(t,b)}
                      />
                    </td>
                  ))}
                </tr>
